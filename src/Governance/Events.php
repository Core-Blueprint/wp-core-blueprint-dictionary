...
final class Events {
	public const CREATED          = 'dictionary.entry.created';
	public const PUBLISHED        = 'dictionary.entry.published';
	public const UPDATED          = 'dictionary.entry.updated';
	public const TRASHED          = 'dictionary.entry.trashed';
	public const RESTORED         = 'dictionary.entry.restored';
	public const DELETED          = 'dictionary.entry.deleted';
	public const SETTINGS_UPDATED = 'dictionary.settings.updated';
	...
	public static function register(): void {
		EventRegistry::register( [ 'id' => self::CREATED, 'label' => __( 'Dictionary entry created', 'core-blueprint-dictionary' ), 'retention_category' => 'general' ] );
		EventRegistry::register( [ 'id' => self::PUBLISHED, 'label' => __( 'Dictionary entry published', 'core-blueprint-dictionary' ), 'retention_category' => 'general' ] );
		EventRegistry::register( [ 'id' => self::UPDATED, 'label' => __( 'Dictionary entry updated', 'core-blueprint-dictionary' ), 'retention_category' => 'general' ] );
		EventRegistry::register( [ 'id' => self::TRASHED, 'label' => __( 'Dictionary entry trashed', 'core-blueprint-dictionary' ), 'retention_category' => 'general' ] );
		EventRegistry::register( [ 'id' => self::RESTORED, 'label' => __( 'Dictionary entry restored', 'core-blueprint-dictionary' ), 'retention_category' => 'general' ] );
		EventRegistry::register( [ 'id' => self::DELETED, 'label' => __( 'Dictionary entry permanently deleted', 'core-blueprint-dictionary' ), 'retention_category' => 'general' ] );
		EventRegistry::register( [ 'id' => self::SETTINGS_UPDATED, 'label' => __( 'Dictionary settings updated', 'core-blueprint-dictionary' ), 'retention_category' => 'settings' ] );
	}

	public static function record_settings_updated( string $field, string $before, string $after ): void {
		Audit::record( self::SETTINGS_UPDATED, 'notice', [ 'field' => sanitize_key( $field ), 'before' => sanitize_text_field( $before ), 'after' => sanitize_text_field( $after ) ] );
	}

	public static function capture_post_update( int $post_id, \WP_Post $post_after, \WP_Post $post_before ): void {
		if ( ! self::is_entry( $post_after ) || self::is_noise( $post_id ) ) {
			return;
		}
		...
		if ( $post_before->post_status !== $post_after->post_status && 'publish' !== $post_after->post_status && 'trash' !== $post_after->post_status && 'trash' !== $post_before->post_status ) {
	...
	public static function capture_insert( int $post_id, \WP_Post $post, bool $update, ?\WP_Post $post_before ): void {
		unset( $post_before );
		if ( $update || ! self::is_entry( $post ) || 'auto-draft' === $post->post_status || self::is_noise( $post_id ) ) {
			return;
		}
		self::mark_created( $post_id );
	}
	...
	public static function capture_terms_change( int $object_id, mixed $terms, array $tt_ids, string $taxonomy, bool $append, array $old_tt_ids ): void {
		...
		$field = match ( $taxonomy ) {
			Taxonomies::CATEGORY => 'categories',
			Taxonomies::TAG      => 'tags',
			Taxonomies::LETTER   => 'alphabet',
			default              => '',
		};
		if ( '' !== $field ) {
			self::mark_field( $object_id, $field );
		}
	}

	public static function capture_status_transition( string $new_status, string $old_status, \WP_Post $post ): void {
		if ( ! self::is_entry( $post ) || self::is_noise( (int) $post->ID ) ) {
			return;
		}
	...
	public static function record_trashed( int $post_id, string $previous_status = '' ): void {
		...
		if ( ! $post instanceof \WP_Post || ! self::is_entry( $post ) ) {
			return;
		}
	...
	public static function flush_changes(): void {
		foreach ( self::$changes as $post_id => $change ) {
			$post = get_post( $post_id );
			if ( ! $post instanceof \WP_Post || ! self::is_entry( $post ) || 'auto-draft' === $post->post_status || 'trash' === $post->post_status ) {
				continue;
			}
			if ( $change['created'] ) {
			...
			sort( $fields );
			if ( ! empty( $fields ) ) {
				Audit::record( self::UPDATED, 'notice', self::context( $post ) + [ 'changed_fields' => $fields ] );
			}
		}
		self::$changes = [];
		self::$lifecycle = [];
	...
	private static function is_entry( \WP_Post $post ): bool {
	...
	/** @return array{entry_id:int,status:string} */
	private static function context( \WP_Post $post ): array {
		return [ 'entry_id' => (int) $post->ID, 'status' => sanitize_key( (string) $post->post_status ) ];
	}
}
